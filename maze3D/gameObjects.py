...
class GameBoard:
    ...
    def __init__(self, layout):
        ...
        for row in range(self.rows):
            ...
            for col in range(self.columns):
                ...
                if layout[row][col] != 0:
                    ...
                    if layout[row][col] == 2:
                        # self.hole = Hole((32 * col) - 530, (32 * row) - 530, self)
                        pass
                    if layout[row][col] == 3:
                        self.ball = Ball((32 * col) - 150, (32 * row) - 150, self)
                        self.hole = Hole((32 * col) - 530, (32 * row) - 530, self)
                        # self.hole = Ball((32 * col) + 160 + 8, (32 * row) + 160 + 8, self)
                    if layout[row][col] != 2 and layout[row][col] != 3:
        ...
        # index:key   values:angle_increment
    ...
    def handleKeys2Old(self, key):
        if key in self.keyMap:
            angleIncrement = self.keyMap[key]
            # print(angleIncrement)
            self.rot_x += 0.09 * angleIncrement[0]
            if self.rot_x >= self.max_x_rotation:
                self.rot_x = self.max_x_rotation
            elif self.rot_x <= -self.max_x_rotation:
                self.rot_x = -self.max_x_rotation
            self.rot_y += 0.09 * angleIncrement[1]
            if self.rot_y >= self.max_y_rotation:
                self.rot_y = self.max_y_rotation
            elif self.rot_y <= -self.max_y_rotation:
                self.rot_y = -self.max_y_rotation
